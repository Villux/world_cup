...
root <- "../data/generated/player_data"
for (file in list.files(root)) {
  date <- substr(file, 12, 21)
  print(date)
  path <- paste(root, file, sep="/")
  player_attributes <- read_csv(path)
...
write.csv(summary, file = "data/generated/team_level_player_data_daily.csv")
